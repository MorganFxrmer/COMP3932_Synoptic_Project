AI Fitness Trainer README
This repository contains the source code for the AI Fitness Trainer, a Kivy-based Python application that uses MediaPipe for pose estimation and an LSTM model to analyse exercise form and predict fatigue levels. The application provides real-time feedback on exercise form and fatigue, with a user-friendly interface for both live camera input and video file analysis. This README provides comprehensive instructions for setting up the development environment, installing dependencies, running the application, and compiling it into an Android APK using Buildozer.
Table of Contents
...
Install Dependencies:With the virtual environment activated, run:
pip install -r requirements.txt


Verify Installation:Ensure all packages are installed correctly by running:
...
On Linux, you may need to install additional system dependencies for Kivy:sudo apt-get install python3-dev libsdl2-dev libsdl2-image-dev libsdl2-mixer-dev libsdl2-ttf-dev
...
Verify Model Files:Ensure the following files exist:
...
Install Buildozer:With the virtual environment activated, run:
...
Set Up Java Environment:Ensure the JAVA_HOME environment variable is set:
...
Initialise Buildozer:In the project root, run:
buildozer init

This creates a buildozer.spec file.

Edit the buildozer.spec File:Open buildozer.spec in a text editor and update the following sections:
[app]
...
Connect an Android Device (optional, for debugging):Enable USB debugging on your Android device and connect it to your computer. Verify the connection:
adb devices


Run Buildozer:Compile the APK in debug mode:
...
Locate the APK:The compiled APK is located in the bin/ directory, e.g.:
bin/aifitnesstrainer-1.0-armeabi-v7a-debug.apk


Install the APK (optional):Install the APK on a connected Android device:
...
Docker Alternative: If you’re not using Linux, run Buildozer in a Docker container:docker run -v $(pwd):/home/user/hostcwd kivy/buildozer
